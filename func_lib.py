...
def create_hist_prices(start_date='2000-01-01', end_date='2024-05-01', min_required_obs=100):
    ...
    # Download historical data with Adj Close included
    historical_prices = yf.download(
        tickers=sp500_tickers,  # Fixed: was using undefined test_tickers
        start=start_date,
        end=end_date,
        auto_adjust=False,  
        group_by=None,  # Changed: from 'ticker' to None
        progress=True
    )
    
    # Ensure 'Adj Close' is present in the second level of the columns
    if 'Adj Close' not in historical_prices.columns.get_level_values('Price'):  # Changed: level name from 0 to 'Price'
        raise ValueError("'Adj Close' not found in downloaded data. Ensure auto_adjust=False is set.")
    
    # Extract only the 'Adj Close' prices
    historical_prices = historical_prices.loc[:, historical_prices.columns.get_level_values('Price') == 'Adj Close']  # Changed: level name
    historical_prices.columns = historical_prices.columns.droplevel('Price')  # Changed: level name from 0 to 'Price'
    historical_prices = historical_prices.reindex(sorted(historical_prices.columns), axis=1)
    
    # Filter tickers by minimum required number of observations
    ticker_counts = historical_prices.count()
    valid_tickers_mask = ticker_counts[ticker_counts >= min_required_obs].index
